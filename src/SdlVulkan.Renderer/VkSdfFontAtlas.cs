    ...
    private GlyphInfo InsertRasterized(GlyphKey key, SdfGlyphBitmap bitmap)
    {
        ...
        if (_cursorY + glyphHeight > _atlasHeight)
        {
            if (_atlasWidth < _maxAtlasSize || _atlasHeight < _maxAtlasSize)
            {
                ...
                // InsertRasterized runs mid-frame (prewarm, on-demand GetGlyph during a draw, disk-load
                // drain); destroying + recreating the VkImage and rebinding its descriptor while the
                // frame's command buffer already has draws bound to the OLD image makes the Adreno tiler
                // fail the next vkQueueSubmit (VK_ERROR_INITIALIZATION_FAILED). BeginFrame grows before
                // any draw is recorded. This glyph lands a frame later — the caller treats a default
                // (Width=0) GlyphInfo as not-yet-available, exactly like the deferred-eviction path below.
                if (!_needsGrow)
                    RenderDiag.Log("sdf.growpending", $"atlas {_atlasWidth}x{_atlasHeight} full glyphs={_glyphs.Count} — deferring grow to next frame");
                _needsGrow = true;
                return default;
            }
