...
- keep the COMMENTS in `src/Directory.Packages.props`. The file itself is upstream's, in upstream's location — it moved up from `src/SdlVulkan.Renderer/` so it covers the test, inspector and webview projects too, which is what let those three drop the `ManagePackageVersionsCentrally=false` each carried to avoid inheriting the *consumer's* CPM through the submodule layout. What stays fork-only is the prose: the `DIR.Lib` line is inert here (the csproj reaches DIR.Lib through the submodule `ProjectReference`) and the `SharpAstro.AppShell` line beside it is not, and that distinction is exactly the knowledge a wholesale overwrite discards. Keep the `DIR.Lib` version in step with the submodule pin rather than letting it rot;
...
`SdlVulkanApp` (process-wide SDL lifecycle + shared `VkInstance` and `VulkanDevice` for a multi-window app) → `SdlVulkanWindow` (per-window SDL3 window + Vulkan surface) → `VulkanContext` (per-window swapchain, command buffers, per-frame sync with `MaxFramesInFlight = 2`, vertex ring, `CurrentFrame` exposed for side-cars; references a `VulkanDevice`) → `VkRenderer` (2D draw API: rectangles, ellipses, lines, text, textures, plus depth-tested meshes) → `VkPipelineSet` (pipelines built from pre-baked SPIR-V — flat, textured, ellipse, page, stroke, SDF, round-rect, blend variants, mesh)

**Device vs. context split (6.0+):** device-level state (logical device, queue, command pool, render pass, descriptor pool/layout, pipeline layout, MSAA, depth format) lives in `VulkanDevice`; per-window state (swapchain, framebuffers, sync, vertex ring, command buffers) lives in `VulkanContext`. `VulkanContext` forwards the device-level members (`ctx.RenderPass`, `ctx.PipelineLayout`, …) so existing consumers keep working. Three construction paths:
...
- `VulkanDevice.cs` — instance/device/queue/render-pass creation (`CreateCompatibleRenderPass`, the one shape every pass takes) and the descriptor-pool chain; shared across windows, so a torn-out window reuses one device.
